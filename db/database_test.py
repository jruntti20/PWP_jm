...
#tests all the constraints in the Member table, hourly_cost should be >= 0
def test_member_constraints(db_handle):
    member = Members(name="uusi ukko", 
                      hourly_cost=-1)
    db_handle.session.add(member)
    with pytest.raises(IntegrityError):
        db_handle.session.commit()
...
def test_all_cost_tests(db_handle):
    #test adds new cost to database
    cost = _get_cost()
    db_handle.session.add(cost)
    db_handle.session.commit()
    ...
    #test removes a cost from the database
    db_handle.session.delete(cost)
    assert Costs.query.count() == 0
    #this works

    #tests onDelete, by deleting the project and phase from database
    project = _get_project()
    phase = _get_phase()
    cost = _get_cost()
    cost.project = project
    ...
    #tests onModify, by changing the project and phase
    project = _get_project()
    project.name = "p1"
    phase = _get_phase()
    phase.name = "phase1"
    ...
    db_handle.session.add(cost)
    db_handle.session.commit()
    assert Costs.query.first().project.name == "p1"
    assert Costs.query.first().phase.name == "phase1"
    cost.project = project2
    cost.phase = phase2
    db_handle.session.add(cost)
    db_handle.session.commit()
    assert Project.query.filter_by(name="p2").first().costs[0].name == "testi cost"
    assert Phase.query.filter_by(name="phase2").first().costs[0].name == "testi cost"
    #this works

#tests all the constraints in the Costs table, hourly_price, quantity and total_costs should be >= 0
def test_costs_constraints(db_handle):
    cost = Costs(name="uusi kustannus", 
                 hourly_price=-1,
                 quantity=-1,
                 total_costs=-1)
    db_handle.session.add(cost)
